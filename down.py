...
def down():
    try:
        inp = input('请输入复制的抖音分享链接：')  # 如：https://v.douyin.com/dMTkJfu/
        url = re.findall('https://v.douyin.com/.*?/', inp)[0]  # 链接解析
        res = requests.get(url)
        vid = re.findall('/video/(\d*)?', res.url)[0]  # vid解析，vid就是视频id，是我自己定义的
        api = f'https://www.iesdouyin.com/web/api/v2/aweme/iteminfo/?item_ids={vid}' # 调用api
        res = requests.get(api, headers={'user-agent': 'chrome'}).json()
        ...
        total_size = int(int(res.headers["Content-Length"])/1024/1024)+1
